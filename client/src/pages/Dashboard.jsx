...
function Dashboard({ user }) {
  ...
  const handleDownloadMitigationReport = async () => {
    ...
    try {
      ...
      setMitigationReport(reportText);
      
      // Give React time to render the hidden markdown
      setTimeout(() => {
        ...
        const opt = {
          margin:       10,
          filename:     'ChainMind_Mitigation_Report.pdf',
        ...
        html2pdf().from(element).set(opt).save().then(() => {
          setDownloadingPDF(false);
          setMitigationReport(''); // cleanup
        });
      }, 500);
      
    } catch (error) {
      console.error('Failed to generate PDF:', error);
      alert('Failed to generate PDF mitigation report.');
      setDownloadingPDF(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-950 font-sans text-slate-200">
      ...
      <div style={{ display: 'none' }}>
        <div id="pdf-content" className="p-8 bg-white text-black font-sans" style={{ minHeight: '100%', width: '800px' }}>
          <div className="border-b border-gray-200 pb-4 mb-6">
            <h1 className="text-3xl font-bold text-blue-600 m-0">ChainMind Mitigation Report</h1>
            <p className="text-gray-500 mt-2 text-sm">Automated Supply Chain Response Protocol</p>
          </div>
          <div className="prose max-w-none text-black">
            <ReactMarkdown>{mitigationReport}</ReactMarkdown>
          </div>
          <div className="mt-12 pt-4 border-t border-gray-200 text-center text-gray-400 text-xs">
            Generated by ChainMind Intelligence
          </div>
        </div>
      </div>
